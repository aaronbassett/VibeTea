...
/// Actions that can result from user input on the dashboard screen.
///
/// This enum represents all possible user intentions that can be expressed
/// through keyboard input while viewing the event stream dashboard. The main
/// event loop processes these actions to update scroll position and auto-scroll
/// behavior accordingly.
///
/// # Action Categories
///
/// - **Line Scrolling**: [`ScrollUp`](DashboardAction::ScrollUp), [`ScrollDown`](DashboardAction::ScrollDown)
/// - **Page Scrolling**: [`ScrollPageUp`](DashboardAction::ScrollPageUp), [`ScrollPageDown`](DashboardAction::ScrollPageDown)
/// - **Jump Navigation**: [`ScrollToTop`](DashboardAction::ScrollToTop), [`ScrollToBottom`](DashboardAction::ScrollToBottom)
/// - **Application Control**: [`Quit`](DashboardAction::Quit)
/// - **No-op**: [`None`](DashboardAction::None)
///
/// # Auto-Scroll Behavior
///
/// Per FR-011, the event stream auto-scrolls to show new events unless the user
/// has manually scrolled up. The following actions affect auto-scroll:
///
/// - **Disable auto-scroll**: `ScrollUp`, `ScrollPageUp`, `ScrollToTop`
/// - **Resume auto-scroll**: `ScrollToBottom`
/// - **No change**: `ScrollDown`, `ScrollPageDown`, `Quit`, `None`
///
/// # Example
///
/// ```
/// use vibetea_monitor::tui::input::DashboardAction;
///
/// let action = DashboardAction::ScrollUp;
/// assert_eq!(action, DashboardAction::ScrollUp);
///
/// let action = DashboardAction::ScrollToBottom;
/// assert_eq!(action, DashboardAction::ScrollToBottom);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DashboardAction {
    /// Scroll the event stream up by one line.
    ///
    /// This action disables auto-scroll, allowing the user to review
    /// historical events without being interrupted by new arrivals.
    ScrollUp,

    /// Scroll the event stream down by one line.
    ///
    /// This action does not change auto-scroll state. If auto-scroll is
    /// disabled, it remains disabled; if enabled, it remains enabled.
    ScrollDown,

    /// Scroll the event stream up by one page.
    ///
    /// A "page" is typically the visible height of the event stream widget.
    /// This action disables auto-scroll.
    ScrollPageUp,

    /// Scroll the event stream down by one page.
    ///
    /// A "page" is typically the visible height of the event stream widget.
    /// This action does not change auto-scroll state.
    ScrollPageDown,

    /// Scroll to the oldest event (top of the buffer).
    ///
    /// This action disables auto-scroll, allowing the user to review
    /// the full event history from the beginning.
    ScrollToTop,

    /// Scroll to the newest event (bottom of the buffer).
    ///
    /// This action re-enables auto-scroll, resuming the default behavior
    /// where new events are automatically shown as they arrive.
    ScrollToBottom,
...
/// Processes a key event in the context of the dashboard screen.
///
/// Translates raw keyboard input into semantic actions for navigating
/// the event stream and controlling the application. The dashboard
/// supports vim-style navigation keys alongside standard arrow keys.
///
/// # Arguments
///
/// * `key_event` - The keyboard event from crossterm
...
/// # Returns
///
/// A [`DashboardAction`] representing the user's intent.
///
/// # Key Mappings
///
/// | Key | Action |
...
/// # Auto-Scroll Behavior
///
/// Per FR-011, scrolling up (ScrollUp, ScrollPageUp, ScrollToTop) disables
/// auto-scroll, allowing the user to review historical events. Scrolling to
/// the bottom (ScrollToBottom) re-enables auto-scroll.
///
/// # Example
///
/// ```
/// use crossterm::event::{KeyCode, KeyEvent, KeyModifiers};
/// use vibetea_monitor::tui::input::{DashboardAction, handle_dashboard_key};
///
/// // Vim-style navigation
/// let key = KeyEvent::new(KeyCode::Char('j'), KeyModifiers::NONE);
/// assert_eq!(handle_dashboard_key(key), DashboardAction::ScrollDown);
///
/// let key = KeyEvent::new(KeyCode::Char('k'), KeyModifiers::NONE);
/// assert_eq!(handle_dashboard_key(key), DashboardAction::ScrollUp);
///
...
/// // Quit with 'q'
/// let key = KeyEvent::new(KeyCode::Char('q'), KeyModifiers::NONE);
/// assert_eq!(handle_dashboard_key(key), DashboardAction::Quit);
/// ```
#[must_use]
pub fn handle_dashboard_key(key_event: KeyEvent) -> DashboardAction {
    match key_event.code {
        // Quit actions
        ...
        // Line scrolling (vim-style and arrow keys)
        KeyCode::Up | KeyCode::Char('k') => DashboardAction::ScrollUp,
        KeyCode::Down | KeyCode::Char('j') => DashboardAction::ScrollDown,

        // Page scrolling
        KeyCode::PageUp => DashboardAction::ScrollPageUp,
        KeyCode::PageDown => DashboardAction::ScrollPageDown,

        // Jump navigation
        KeyCode::Home | KeyCode::Char('g') => DashboardAction::ScrollToTop,
        KeyCode::End | KeyCode::Char('G') => DashboardAction::ScrollToBottom,
...
mod tests {
    ...
    #[test]
    fn dashboard_action_is_debug() {
        let action = DashboardAction::ScrollUp;
        let debug_str = format!("{:?}", action);
        assert!(debug_str.contains("ScrollUp"));
    }
    ...
    #[test]
    fn dashboard_action_is_copy() {
        let action = DashboardAction::ScrollPageUp;
        let copied = action;
        // Can still use original after copy (Copy trait)
        assert_eq!(action, copied);
    }

    #[test]
    fn dashboard_action_is_eq() {
    ...
    #[test]
    fn dashboard_action_variants_are_distinct() {
        let variants = [
            DashboardAction::ScrollUp,
            DashboardAction::ScrollDown,
            DashboardAction::ScrollPageUp,
            DashboardAction::ScrollPageDown,
        ...
        // Each variant should be distinct from others
        for (i, a) in variants.iter().enumerate() {
            for (j, b) in variants.iter().enumerate() {
                if i == j {
                    assert_eq!(a, b);
    ...
    // =========================================================================
    // Dashboard Quit Key Tests
    // =========================================================================

    #[test]
    fn dashboard_q_quits() {
        let key = KeyEvent::new(KeyCode::Char('q'), KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::Quit);
    }

    #[test]
    fn dashboard_escape_quits() {
        let key = KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::Quit);
    }
    ...
    // =========================================================================
    // Dashboard Line Scrolling Tests (Vim-style and Arrow Keys)
    // =========================================================================

    #[test]
    fn dashboard_up_arrow_scrolls_up() {
        let key = KeyEvent::new(KeyCode::Up, KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::ScrollUp);
    }
    ...
    #[test]
    fn dashboard_down_arrow_scrolls_down() {
        let key = KeyEvent::new(KeyCode::Down, KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::ScrollDown);
    }

    #[test]
    fn dashboard_j_scrolls_down() {
        let key = KeyEvent::new(KeyCode::Char('j'), KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::ScrollDown);
    }
    ...
    // =========================================================================
    // Dashboard Page Scrolling Tests
    // =========================================================================

    #[test]
    fn dashboard_page_up_scrolls_page_up() {
        let key = KeyEvent::new(KeyCode::PageUp, KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::ScrollPageUp);
    }

    #[test]
    fn dashboard_page_down_scrolls_page_down() {
        let key = KeyEvent::new(KeyCode::PageDown, KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::ScrollPageDown);
    }

    // =========================================================================
    // Dashboard Jump Navigation Tests
    // =========================================================================

    #[test]
    fn dashboard_home_scrolls_to_top() {
        let key = KeyEvent::new(KeyCode::Home, KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::ScrollToTop);
    }

    #[test]
    fn dashboard_g_scrolls_to_top() {
    ...
    #[test]
    fn dashboard_end_scrolls_to_bottom() {
    ...
    #[test]
    fn dashboard_capital_g_scrolls_to_bottom() {
        let key = KeyEvent::new(KeyCode::Char('G'), KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::ScrollToBottom);
    }
    ...
    // =========================================================================
    // Dashboard Unhandled Key Tests
    // =========================================================================

    #[test]
    fn dashboard_letter_returns_none() {
    ...
    #[test]
    fn dashboard_enter_returns_none() {
        let key = KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::None);
    }
    ...
    #[test]
    fn dashboard_space_returns_none() {
        let key = KeyEvent::new(KeyCode::Char(' '), KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::None);
    }
    ...
    #[test]
    fn dashboard_ctrl_k_returns_none() {
        // Control modifier should not trigger vim scrolling
        let key = KeyEvent::new(KeyCode::Char('k'), KeyModifiers::CONTROL);
        let action = handle_dashboard_key(key);
        // Note: The match pattern doesn't check modifiers for k/j
        // This tests current behavior; adjust if modifier checking is added
        assert_eq!(action, DashboardAction::ScrollUp);
    }

    #[test]
    fn dashboard_shift_g_still_scrolls_to_bottom() {
        // Shift+g produces 'G' which should scroll to bottom
    ...
    #[test]
    fn dashboard_left_arrow_returns_none() {
        // Left/Right arrows are not mapped on dashboard
        let key = KeyEvent::new(KeyCode::Left, KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::None);
    }
    ...
    #[test]
    fn dashboard_right_arrow_returns_none() {
        let key = KeyEvent::new(KeyCode::Right, KeyModifiers::NONE);
        let action = handle_dashboard_key(key);
        assert_eq!(action, DashboardAction::None);
    }
